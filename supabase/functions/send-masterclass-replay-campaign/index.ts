...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("Sending masterclass replay campaign email:", { 
      email: body.email, 
      name: body.name,
      testMode: body.testMode 
    });

    const template = getEmailTemplate(body.name);
    ...
    // Track email send in database (unless test mode)
    if (!body.testMode) {
      ...
      // Find lead by email to link the send
      ...
      if (lead && sequence) {
        ...
        console.log("Email send tracked for lead:", lead.id);
      }
    }
